...
// API Configuration
const API_BASE_URL = 'https://cyber-saathi-backend-1.onrender.com';
  ...
  async getHealth(): Promise<HealthResponse> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new ApiError(
          errorData.error || 'Failed to get health status from backend',
          response.status
        );
      }
  ...
  async sendChatMessage(messages: Message[]): Promise<ChatResponse> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new ApiError(
          errorData.error || 'Failed to get response from backend',
          response.status
        );
      }
      const data = await response.json();
  ...
  async getNews(): Promise<NewsResponse> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new ApiError(
          errorData.error || 'Failed to get news from backend',
          response.status
        );
      }
      const data = await response.json();
      console.log(
        'News fetch successful, articles count:',
  ...
  async getBooklets(): Promise<BookletsResponse> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new ApiError(
          errorData.error || 'Failed to get booklets from backend',
          response.status
        );
      }
  ...
  async scanUrl(url: string): Promise<UrlScanResponse> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new ApiError(
          errorData.error || 'Failed to scan URL',
          response.status
        );
      }
      const data = await response.json();
